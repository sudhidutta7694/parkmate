...
export class ValetAssignmentsResolver {
  ...
  async updateValetAssignment(
    ...
    if (!valetAssignment) {
      throw new Error('Valet assignment not found')
    }
    checkRowLevelPermission(
      user,
      [valetAssignment.pickupValetId, valetAssignment.returnValetId].filter(
  ...
  async removeValetAssignment(
    ...
    const valetAssignment = await this.prisma.valetAssignment.findUnique({
      where:
        args.where as unknown as import('../../../../generated/prisma').Prisma.ValetAssignmentWhereUniqueInput,
    })
    if (!valetAssignment) {
      throw new Error('Valet assignment not found')
    }
